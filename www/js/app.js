...
const App = (function () {
  ...
  function init() {
    ...
    // 7. 迁移旧版明文 API Key → 加密存储（异步，不阻塞启动）
    SecureStore.migrateLegacyKey();
  ...
  function renderSettingsPage() {
    ...
    if (apiKeyEl) {
      // API Key 加密存储，不回显明文，只显示占位提示
      apiKeyEl.value = '';
      if (SecureStore.hasEncryptedKey()) {
        apiKeyEl.placeholder = '🔒 已加密保存，无需重新输入';
        apiKeyEl.dataset.hadKey = '1';
      } else {
        apiKeyEl.placeholder = 'sk-…';
        delete apiKeyEl.dataset.hadKey;
      }
    }
  ...
  async function saveAISettings() {
    ...
    if (apiKey) {
      var inputValue = apiKey.value.trim();
      if (inputValue) {
        // 用户输入了新 Key → 加密保存
        var encrypted = await SecureStore.saveApiKey(inputValue);
        apiKey.value = '';
        apiKey.placeholder = '🔒 已加密保存，无需重新输入';
        apiKey.dataset.hadKey = '1';
        showToast(encrypted ? '🔒 API Key 已加密保存' : '⚠️ 当前环境不支持加密，已明文保存');
      } else if (apiKey.dataset.hadKey === '1') {
        // 用户清空了输入框 → 清除已保存的 Key
        await SecureStore.saveApiKey('');
        delete apiKey.dataset.hadKey;
  ...
  async function testAIConnection() {
    ...
    var config = Storage.getAIConfig();
    var apiKey = await SecureStore.getApiKey();

    if (!config.endpoint || !apiKey) {
      showToast('⚠️ 请先填写 API Key');
      return;
    }

    showToast('🔌 正在测试连接…');

    // 使用解密后的 Key 测试（只存在于内存，不写回存储）
    var testConfig = Object.assign({}, config, { apiKey: apiKey });
